...
class UserDevices extends ControllerBase {
  ...
  protected function _getData($header) {
   /* if (@inet_pton($host) === FALSE) //inet_pton занимается проверкой IPv6 адресов на соответствие
    ...
    $query->condition('d.devperson_ip');
    ...
    foreach ($results as $data) {
      ...
      $modanalys_host_link = \Drupal::l($this->t('details'),$modanalys_host_url,\Drupal\Core\Url::fromRoute('modanalys.hit_details',array("devperson_id"=>$data->devperson_id)));
