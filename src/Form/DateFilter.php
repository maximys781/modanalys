...
class DateFilter extends FormBase {
    ...
    public function validateForm(array &$form, FormStateInterface $form_state) {
        ...
        $error_message = $this->t('Эта дата введена некорректно');
    ...
    public function datelistValidate($element, FormStateInterface $form_state) {
        ...
        if (empty($input['year']) || empty($input['month']) || empty($input['day'])) {
            $form_state->setError($element,$this->t('Введите %field даты.'));
            return;
        }

        if (!checkdate($input['month'], $input['day'], $input['year'])) {
            $this->setError($element,$this->t('Некорректный ввод даты.'));
            return;
        }
        ...
        $form_state->setError($element,$this->t(' %field даты некорректно.'));
    }
